...
const tools = [
  ...
  {
    name: 'prism_read',
    description: 'Read the value at a PATH inside a JSON/JSONL blob, token-budgeted. Paths look like data.users[0].email or items[*].id ([*] maps over an array). If the value fits the budget you get it verbatim; if it is too big you get its shape plus the honest size, so you can narrow the path and read again — never a silent truncation. A wrong path is reported (where it fell off, and the type actually there), not thrown.',
    inputSchema: { type: 'object', properties: {
      ...SRC_PROPS,
      path: { type: 'string', description: 'The path to read, e.g. "data.items[0].name", "users[*].id", or "$" for the root.' },
